/**
 * providerService 单元测试：目标平台匹配、平台条目 discover 映射。
...
function stubJson(payload: unknown): void {
  fetchHandler = () => ({ ok: true, json: async () => payload });
}

describe('providerService', () => {
  it('从 watch/providers 列表匹配出美区目标平台并带 logo', async () => {
    stubJson({
      results: [
        { provider_id: 8, provider_name: 'Netflix', logo_path: '/net.png', display_priority: 1 },
        { provider_id: 350, provider_name: 'Apple TV+', logo_path: '/apl.png', display_priority: 2 },
        { provider_id: 9, provider_name: 'Prime Video', logo_path: '/pri.png', display_priority: 3 },
        { provider_id: 337, provider_name: 'Disney+', logo_path: '/dis.png', display_priority: 4 },
        { provider_id: 15, provider_name: 'Hulu', logo_path: '/hul.png', display_priority: 5 },
        { provider_id: 1899, provider_name: 'Max', logo_path: '/max.png', display_priority: 6 },
        { provider_id: 999, provider_name: 'Random TV', logo_path: null, display_priority: 90 },
      ],
    });
    ...
    assert.deepEqual(
      us.providers.map((p) => p.key),
      ['netflix', 'apple-tv', 'prime', 'disney', 'hulu', 'max'],
    );
    assert.equal(us.providers[0].id, 8);
    assert.ok(us.providers[0].logoPath?.startsWith('https://image.tmdb.org/t/p/w92'));
    // 同请求同时用于国区匹配：国区列表中无中文平台则国区为空组（不报错）
    assert.ok(regions.find((r) => r.key === 'cn'));
  });
  ...
  it('国区按中文名匹配爱奇艺/腾讯视频', async () => {
    ...
    stubJson({
      results: [
        { provider_id: 18, provider_name: '爱奇艺', logo_path: '/iq.png', display_priority: 1 },
        { provider_id: 22, provider_name: '腾讯视频', logo_path: '/tx.png', display_priority: 2 },
      ],
    });
    const regions = await listProviderRegions();
    ...
    assert.equal(cn.providers[0].id, 18);
  });
